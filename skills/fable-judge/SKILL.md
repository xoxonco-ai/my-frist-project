...
`/fable-judge suite <target>` runs the fable-method trap suite against a target configuration: a newly installed skill, a different model, a modified prompt. It needs the fable-method `eval/` directory, which is **not** bundled with this skill in the marketingskills marketplace. To run suite mode, clone `https://github.com/Sahir619/fable-method` and use the `eval/` directory there. (If instead you installed the fable-method plugin directly from that repo, `eval/` sits at the plugin root, one level above the `skills/` directory.)
